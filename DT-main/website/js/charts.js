...
(function () {
          ...
          legend: {
            ...
            labels: {
              filter: function (item, chartData) {
                return chartData.datasets[item.datasetIndex].stack === 'stack0';
              },
              boxWidth: 10,
              boxHeight: 10,
              padding: 10,
  ...
  function updateDispatchChart(chart, simResults, options) {
    ...
    // Stacked area per source — order: 1 (gas, bottom) … 6 (liquid_fuel, top)
    // Lower order = stacked first (visual bottom); demand/reqcap use order>6 to draw on top.
    SAED.STACK_ORDER.forEach(src => {
      ...
      datasets.push({
        ...
        tension: 0.3,
        stack: 'stack0',
        order: SAED.STACK_ORDER.indexOf(src) + 1
      });
    });

    // Demand line — order:10 ensures it's drawn on top of all stacked fills
    datasets.push({
      label: 'Demand',
      data: HOURS.map(h => {
        const row = seasonData.find(r => r.hour === h);
    ...
    // Required capacity line — order:9, drawn above stacked fills, below demand
    datasets.push({
